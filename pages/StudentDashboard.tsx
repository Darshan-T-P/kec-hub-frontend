        ...
        <>
            <header className="flex flex-col md:flex-row md:items-center justify-between gap-6 bg-white p-8 rounded-[2rem] border border-slate-100 shadow-sm">
                ...
                <div className="flex gap-3">
                    ...
                    >
                        {isCrawling ? 'Discovering...' : 'Discover Live Opportunities'}
                        ...
                        {discoveredOpps.length > 0 ? discoveredOpps.map((opp) => (
                            <div
                                ...
                                onClick={() => {
                                    setSelectedOpp(opp);
                                    if (user?.email) {
                                        mlService.sendFeedback(user.email, opp.id, 'clicked');
                                    }
                                }}
                                ...
                                <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
                                    <div className="flex-1">
                                        <div className="flex items-center gap-3 mb-2">
                                            ...
                                            {opp.score && opp.score > 0.7 && (
                                                <span className="px-2 py-0.5 bg-amber-50 text-amber-600 text-[10px] font-black uppercase rounded border border-amber-100">
                                                    Best Match: {Math.round(opp.score * 100)}%
                                                </span>
                                            )}
                                        </div>
                                        ...
                                        {opp.reasons && opp.reasons.length > 0 && (
                                            <div className="flex flex-wrap gap-2 mt-3">
                                                {opp.reasons.map((reason, idx) => (
                                                    <span key={idx} className="text-[9px] font-black text-indigo-400 bg-indigo-50/50 px-2 py-0.5 rounded-full border border-indigo-100/50">
                                                        ✨ {reason}
                                                    </span>
                                                ))}
                                            </div>
                                    ...
                                    <div className="flex flex-col items-end gap-2">
                                        ...
                                        <button
                                            ...
                                            className="text-indigo-600 font-black text-sm group-hover:underline"
                                        >
                                            Quick Apply →
                                        </button>
                                    </div>
                            ...
                            <div className="p-20 text-center bg-slate-50 rounded-[3rem] border-4 border-dashed border-slate-200">
                                ...
                                <p className="text-slate-400 font-bold mt-2 max-w-xs mx-auto">Click Discover Live Opportunities to fetch realtime opportunities matching your profile. If you get 0 results, check your backend configuration.</p>
                            </div>
                        )}
                    </div>
                </div>
